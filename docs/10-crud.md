...
- Secara otomatis semua entity / model yang terdeteksi oleh [EntityTrxManager](./09-trxmanager.md) dapat diakses menggunakan modul ini.
- Untuk membatasi akses dapat dipasangkan dengan ApiRole (lihat: modul [API](./21-api.md)).
...
CrudHandler crudHandler(
    ...
    // Bulk diaktifkan atau tidak
    .setBulkEnabled(crud.bulkEnabled().orElse(null))
    
    // Maksimum jumlah operasi / aksi CRUD dalam satu request bulk yang diijinkan, diisi 0 untuk tak terbatas
    .setMaxBulkSize(crud.maxBulkSize().orElse(null))
    
    // Maksimum jumlah dependensi / layer CRUD dalam satu request bulk yang diijinkan, diisi 0 untuk tak terbatas
...
@Public
@Path("/crud")
class CrudController extends net.ideahut.quarkus.crud.CrudControllerBase {
...
- `CREATE` membuat satu data baru.
- `CREATES` membuat koleksi data baru.
- `UPDATE` memperbaharui satu data yang sudah ada.
- `UPDATES` memperbaharui koleksi data yang sudah ada.
- `SAVE` membuat / memperbaharui satu data.
- `SAVES` membuat / memperbaharui koleksi data.
...
- `values` array value untuk insert, update, dan save.
...
<div>
   <img src="./assets/crud.png" alt="" width="800" />
</div>
