...
* `Home`: melihat informasi aplikasi, versi, dan bean-bean yang tersedia.
* `Reload`: memuat ulang bean tanpa perlu merestart service.
* [`Request`](./02-advice.md#requestinfo-collector): daftar request mapping atau endpoint yang tersedia di service.
* [`Task`](./14-task.md): memantau task yang bekerja di latar belakang.
* [`Redis`](./18-redis.md): memantau dan mengatur redis yang digunakan di service.
* [`Cache`](./13-cache.md): menghapus atau membersihkan data cache baik itu group maupun tunggal.
* [`Grid`](./11-grid.md): mengakses table-table database (operasi [CRUD](./10-crud.md)).
* [`Audit`](./12-audit.md): melihat perubahan-perubahan data (aksi: INSERT, UPDATE, DELETE) dari tabel-tabel database.
* [`Kafka`](./19-kafka.md): memantau dan mengatur kafka yang digunakan di service.
* [`Entity`](./09-trxmanager.md): melihat daftar entity atau model yang tersedia berdasarkan transactionManager, termasuk untuk membuat replica & grid.
* [`Scheduler`](./20-scheduler.md): memantau dan mengatur job, schedule / unschedule job, start / stop scheduler.
...
AdminHandler adminHandler(
   ...
   // Daftar option select yang digunakan di template grid, contoh: GENDER, BOOLEAN, dll
...
central:
   # Untuk mengambil file-file template terbaru yang akan diubah sesuai konfigurasi admin
...
# konfigurasi API admin yang akan diset ke file-file template
api:
   timeout: 30
   debug: true

# konfigurasi Web UI yang akan diset ke file-file template
web:
...
## Role
User secara default bisa mengakses semua fitur admin jika role-nya tidak ada.
``` yaml
VIEWER:
   reload:
      enable: false
