...
TaskHandler taskHandler(
	...
	TaskProperties properties = TaskDefinition.convert(appProperties.task().orElseThrow());
...
```
- `allowCoreThreadTimeOut`: Ijinkan timeout thread saat eksekusi task.
...
- `keepAliveTime`: Timeout thread selama eksekusi task.
...
- `disableVirtualThread`: Tidak menggunakan Virtual Thread meskipun disupport oleh JDK yang digunakan.
- `prestartAllCoreThreads`: Start sejumlah thread sesuai dengan `corePoolSize`.

## Screenshot

<div>
   <img src="./assets/task.png" alt="" width="800" />
</div>
