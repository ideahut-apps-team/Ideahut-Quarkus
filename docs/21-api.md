...
* Menangani data yang tersimpan di database dan [redis](./18-redis.md).
...
ApiHandler apiHandler(
    ...
    // RedisDataSource dan definisi penamaan key-nya
    .setRedisParam(
...
ApiTokenService apiTokenService(
    ...
    // Index redisDataSource untuk meyimpan consumer token jika menggunakan MultipleRedisDataSource, default: 1
...
* Untuk ApiName custom bisa menggunakan setApiName (default diambil dari 'quarkus.application.name').
...
ApiService apiService(
    ...
    // Jika tidak diset akan digunakan ID dari application context (property 'quarkus.application.name')
    ...
    // Cache expiry, untuk data access & consumer baik yang null maupun tidak
	.setCacheExpiry(ApiDefinition.CacheExpiry.convert(service.cacheExpiry().orElse(null)))
...
## ApiCacheExpiry
Untuk menyimpan ApiAccess baik untuk role maupun consumer dalam waktu tertentu. Sehingga ApiService tidak perlu selalu memuat dari database atau sumber data lain di setiap request.

* `timeUnit`: timeUnit yang digunakan untuk menyimpan cache.
* `accessNull`: kadaluarsa ApiAccess role yang null.
* `accessObject`: kadaluarsa ApiAccess role yang bukan null.
* `consumerNull`: kadaluarsa ApiAccess consumer yang null.
* `consumerObject`: kadaluarsa ApiAccess consumer yang bukan null.
...
* Untuk menambah processor lain, dengan cara meng-_extends_ class 'net.ideahut.quarkus.api.ApiProcessor' dan gunakan ApiType yang belum terpakai.
* Daftar default ApiProcessor bisa dilihat di table di bawah (validasi bisa berdasarkan User-Agent & Remote-Host pada saat token dibuat).
...
<div>
   <img src="./assets/api-01.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-02.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-03.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-04.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-05.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-06.png" alt="" width="800" />
</div>
<br/>
<div>
   <img src="./assets/api-07.png" alt="" width="800" />
</div>
